...
describe('should verify signature on random inputs', () => {
    ...
    it('should fail for invalid msg  ascii ', async function () {
        ...
        try {
            ...
            if (!errMsg.includes("Num2Bits_85")) {
                throw new Error(`Expected error message to include "Num2Bits_85", but got:\n${errMsg}`);
            }
            didThrow = true;
        }
        if (!didThrow) {
    ...
    it('should fail for s > 251 bits', async function () {
        ...
        try {
            ...
            const errMsg = e?.message || e?.toString?.() || "";
            if (!errMsg.includes("line: 57")) {
                throw new Error(`Expected error message to include "line: 57", but got:\n${errMsg}`);
            }
            didThrow = true;
        }
