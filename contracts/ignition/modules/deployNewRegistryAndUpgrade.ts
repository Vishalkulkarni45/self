...
export default buildModule("DeployNewHubAndUpgrade", (m) => {
  ...
  if (!registryProxyAddress) {
    throw new Error(
      "Registry proxy address not found in deployed_addresses.json",
    );
  }

  const newRegistryImpl = m.contract(
    "testUpgradedIdentityVerificationHubImplV1",
  );

  const testRegistryInterface = getTestRegistryInitializeData();
  const initializeData = testRegistryInterface.encodeFunctionData(
    "initialize",
    [true],
  );

  const registryProxy = m.contractAt(
