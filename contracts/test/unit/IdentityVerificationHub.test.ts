...
describe("Unit Tests for IdentityVerificationHub", () => {
  ...
  describe("Initialization", () => {
    it("should initialize hub with correct parameters", async () => {
      ...
      expect(await hub.vcAndDiscloseCircuitVerifier()).to.equal(
      ...
      expect(await hub.sigTypeToRegisterCircuitVerifiers(registerId)).to.equal(
        register.target,
      );
      ...
      expect(hubInitializedEvent.args.vcAndDiscloseCircuitVerifier).to.equal(
        vcAndDisclose.target,
      );
      expect(hubInitializedEvent.args.registerCircuitVerifierIds).to.deep.equal(
        [registerId],
      );
      expect(hubInitializedEvent.args.registerCircuitVerifiers).to.deep.equal([
        register.target,
      ]);
      expect(hubInitializedEvent.args.dscCircuitVerifierIds).to.deep.equal([
        dscId,
      ]);
    ...
    it("should revert when register circuit verifier arrays length mismatch", async () => {
      ...
      const initializeData = hubImpl.interface.encodeFunctionData(
        "initialize",
        [registry.target, vcAndDisclose.target, [1], [], [], []],
      );
      const hubProxyFactory = await ethers.getContractFactory(
    ...
    it("should not allow initialization after initialized", async () => {
      ...
      await expect(
        hub.initialize(registry.target, vcAndDisclose.target, [], [], [], []),
  ...
  describe("Update functions", () => {
    ...
    it("should not update registry address if caller is not owner", async () => {
      ...
      await expect(
        hub.connect(user1).updateRegistry(newRegistryAddress),
    ...
    it("should not update registry address if caller is not proxy", async () => {
      ...
      await expect(
        hubImpl.updateRegistry(newRegistryAddress),
    ...
    it("should not update vc and disclose circuit verifier if caller is not owner", async () => {
      ...
      await expect(
        hub.connect(user1).updateVcAndDiscloseCircuit(newVerifierAddress),
      ).to.be.revertedWithCustomError(hub, "OwnableUnauthorizedAccount");
    });
    ...
    it("should not update vc and disclose circuit verifier if caller is not proxy", async () => {
      ...
      await expect(
        hubImpl.updateVcAndDiscloseCircuit(newVerifierAddress),
      ).to.be.revertedWithCustomError(hubImpl, "UUPSUnauthorizedCallContext");
    });
    ...
    it("should update register circuit verifier", async () => {
      ...
      await expect(
        hub.updateRegisterCircuitVerifier(verifierId, newVerifierAddress),
      )
        .to.emit(hub, "RegisterCircuitVerifierUpdated")
        .withArgs(verifierId, newVerifierAddress);

      expect(await hub.sigTypeToRegisterCircuitVerifiers(verifierId)).to.equal(
        newVerifierAddress,
      );
    });
    ...
    it("should not update register circuit verifier if caller is not owner", async () => {
      ...
      await expect(
        hub
          .connect(user1)
          .updateRegisterCircuitVerifier(verifierId, newVerifierAddress),
      ).to.be.revertedWithCustomError(hub, "OwnableUnauthorizedAccount");
    });
    ...
    it("should not update register circuit verifier if caller is not proxy", async () => {
      ...
      await expect(
        hubImpl.updateRegisterCircuitVerifier(verifierId, newVerifierAddress),
      ).to.be.revertedWithCustomError(hubImpl, "UUPSUnauthorizedCallContext");
    });
    ...
    it("should not update DSC verifier if caller is not owner", async () => {
      ...
      await expect(
        hub.connect(user1).updateDscVerifier(verifierId, newVerifierAddress),
      ).to.be.revertedWithCustomError(hub, "OwnableUnauthorizedAccount");
    });
    ...
    it("should not update DSC verifier if caller is not proxy", async () => {
      ...
      await expect(
        hubImpl.updateDscVerifier(verifierId, newVerifierAddress),
      ).to.be.revertedWithCustomError(hubImpl, "UUPSUnauthorizedCallContext");
    });
    ...
    it("should batch update register circuit verifiers", async () => {
      ...
      const newVerifierAddresses = [
        await user1.getAddress(),
        await user1.getAddress(),
      ];

      await expect(
        hub.batchUpdateRegisterCircuitVerifiers(
          verifierIds,
      ...
      for (let i = 0; i < verifierIds.length; i++) {
        expect(
          await hub.sigTypeToRegisterCircuitVerifiers(verifierIds[i]),
    ...
    it("should not batch update register circuit verifiers if caller is not proxy", async () => {
      ...
      await expect(
        hubImpl.batchUpdateRegisterCircuitVerifiers(
          verifierIds,
          newVerifierAddresses,
        ),
      ).to.be.revertedWithCustomError(hubImpl, "UUPSUnauthorizedCallContext");
    ...
    it("should batch update DSC circuit verifiers", async () => {
      ...
      for (let i = 0; i < verifierIds.length; i++) {
        expect(await hub.sigTypeToDscCircuitVerifiers(verifierIds[i])).to.equal(
    ...
    it("should not batch update DSC circuit verifiers if caller is not owner", async () => {
      ...
      await expect(
        hub
          .connect(user1)
          .batchUpdateDscCircuitVerifiers(verifierIds, newVerifierAddresses),
      ).to.be.revertedWithCustomError(hub, "OwnableUnauthorizedAccount");
    });
    ...
    it("should not batch update DSC circuit verifiers if caller is not proxy", async () => {
      ...
      await expect(
        hubImpl.batchUpdateDscCircuitVerifiers(
          verifierIds,
          newVerifierAddresses,
        ),
      ).to.be.revertedWithCustomError(hubImpl, "UUPSUnauthorizedCallContext");
  ...
  describe("View functions", () => {
    ...
    it("should not return when view function is called by non-proxy", async () => {
      ...
      await expect(hubImpl.registry()).to.be.revertedWithCustomError(
        hubImpl,
    ...
    it("should not return when view function is called by non-proxy", async () => {
      const { hubImpl } = deployedActors;
      await expect(
        hubImpl.vcAndDiscloseCircuitVerifier(),
      ).to.be.revertedWithCustomError(hubImpl, "UUPSUnauthorizedCallContext");
    });
    ...
    it("should not return when view function is called by non-proxy", async () => {
      const { hubImpl } = deployedActors;
      await expect(
        hubImpl.sigTypeToRegisterCircuitVerifiers(1),
      ).to.be.revertedWithCustomError(hubImpl, "UUPSUnauthorizedCallContext");
    });
    ...
    it("should not return when view function is called by non-proxy", async () => {
      const { hubImpl } = deployedActors;
      await expect(
        hubImpl.sigTypeToDscCircuitVerifiers(1),
      ).to.be.revertedWithCustomError(hubImpl, "UUPSUnauthorizedCallContext");
    });
  });
  ...
  describe("Upgradeabilitiy", () => {
    it("should preserve state after upgrade", async () => {
      ...
      const vcAndDiscloseCircuitVerifierBefore =
        await hub.vcAndDiscloseCircuitVerifier();
      const registerCircuitVerifierIdsBefore =
        await hub.sigTypeToRegisterCircuitVerifiers(
          RegisterVerifierId.register_sha256_sha256_sha256_rsa_65537_4096,
        );
      const dscCircuitVerifierIdsBefore =
        await hub.sigTypeToDscCircuitVerifiers(
          DscVerifierId.dsc_sha256_rsa_65537_4096,
        );

      const HubV2Factory = await ethers.getContractFactory(
      ...
      const hubV2 = await ethers.getContractAt(
        "testUpgradedIdentityVerificationHubImplV1",
        hub.target,
      );
      ...
      expect(await hubV2.vcAndDiscloseCircuitVerifier()).to.equal(
        vcAndDiscloseCircuitVerifierBefore,
      );
      expect(
        await hubV2.sigTypeToRegisterCircuitVerifiers(
          RegisterVerifierId.register_sha256_sha256_sha256_rsa_65537_4096,
        ),
      ).to.equal(registerCircuitVerifierIdsBefore);
      expect(
        await hubV2.sigTypeToDscCircuitVerifiers(
          DscVerifierId.dsc_sha256_rsa_65537_4096,
        ),
