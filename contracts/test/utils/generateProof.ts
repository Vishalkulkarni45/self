...
export async function generateRegisterProof(
  ...
  const registerCircuitInputs: CircuitSignals =
    await generateCircuitInputsRegister(
      secret,
      passportData,
  ...
  const vKey = JSON.parse(
    fs.readFileSync(
      registerCircuits["register_sha256_sha256_sha256_rsa_65537_4096"].vkey,
      "utf8",
    ),
  );
  ...
  const fixedProof = parseSolidityCalldata(
    rawCallData,
    {} as RegisterCircuitProof,
  );
...
export async function generateVcAndDiscloseRawProof(
  ...
  const vcAndDiscloseCircuitInputs: CircuitSignals =
    generateCircuitInputsVCandDisclose(
      secret,
      attestationId,
      passportData,
  ...
  // Verify the proof
  const vKey = JSON.parse(
    fs.readFileSync(vcAndDiscloseCircuits["vc_and_disclose"].vkey, "utf8"),
  );
  const isValid = await groth16.verify(
    vKey,
